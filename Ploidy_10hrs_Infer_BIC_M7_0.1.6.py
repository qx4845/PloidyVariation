...
v0.1.4 series are actually full data
v0.1.5 series are actually 10hrs data
v0.1.6 series attempts to redefine number of data points for BIC as every time pathProbs is called (instead of every frame)
"""
### Initialize: 
import numpy as np
...
modelNum = 7
version = "Ploidy_10hrs_Infer_BIC_M%s_0.1.6" %(str(int(modelNum)))
...
def MaxCal_MLe_count(lagrangeVals):
    global factMat, cap, numChrom
    dataCounter = 0
    probsList = [[[] for ind_g in range(cap)] for ind_C in range(max_NC + 1)]
    logLike = 0.0
...
initLogLike, totalDataPoints = MaxCal_MLe_count(firstGuess)
...
##BIC = len(MC_lagrange)*np.log(totalExpTrajLen) + 2*Lagr01
#BIC = len(bestGuess.tolist())*np.log(totalExpTrajLen) + 2*bestLike
...
#print('Lagrange Mutlipliers: \n[h_C, h_g, h_P, h_KgC] = ' + str(bestGuess.tolist()) + ' \nyield -log(Likelihood) = ' + str(bestLike) + ", and BIC = " + str(BIC) + '\n')
...
print("\nOptimized Lagrange Multipliers: \n[h_C, h_g, h_P, h_KgC, h_KgC2] = " + str(bestGuess.tolist()) + "\nwith corresponding -log(Likelihood) = " + str(bestLike[0]) + ", and BIC = " + str(BIC[0]))
#print("\nOptimized Lagrange Multipliers: \n[h_C, h_g, h_P] = " + str(bestGuess.tolist()) + "\nwith corresponding -log(Likelihood) = " + str(bestLike[0]) + ", and BIC = " + str(BIC[0]))
...
#filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/BIC_Comparison/10hrs/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_hP=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(np.round(bestGuess[2], 3)), str(timeRes), str(version))
filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/BIC_Comparison/10hrs/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_hP=%s_hKgC=%s_hKgC2=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(np.round(bestGuess[2], 3)), str(np.round(bestGuess[3], 3)), str(np.round(bestGuess[4], 3)), str(timeRes), str(version))
#filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/BIC_Comparison/10hrs/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(timeRes), str(version))
#filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/BIC_Comparison/10hrs/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(timeRes), str(version))

np.savez(filename_inference, bestGuess = bestGuess, bestLike = bestLike, BIC = BIC, bestInd = bestInd, fitStartGuess = fitStartGuess, fitLagrangeVals = fitLagrangeVals, fitLogLike = fitLogLike, div_times_exp = div_times_exp)
