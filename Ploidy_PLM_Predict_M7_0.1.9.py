...
global deltaT, timeRes, t_f, maxT
...
version = "%shr_PLM_Predict_%shr_MCMC_M%s_0.1.9" %(str(int(crop_hrs)), str(int(MCMC_maxT_hours)), str(int(modelNum)))
...
#
#initLogLike = MaxCal_MLe(firstGuess, numChrom)
#
...
#print('Optimized Lagrange Mutlipliers\n(Model # ' + str(int(modelNum)) + ', Truncated data length: ' + str(int(crop_hrs)) + ' hours): \n[h_C, h_g, h_P, h_KgC, h_KgC2] = ' + str(np.round(bestGuess.tolist(), 3)) + ' \nyield -log(Likelihood) = ' + str(np.round(bestLike[0], 3)) + ", and BIC = " + str(np.round(BIC[0], 3)) + '\n')
#
#filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/PartialData_Prediction/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_hP=%s_hKgC=%s_hKgC2=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(np.round(bestGuess[2], 3)), str(np.round(bestGuess[3], 3)), str(np.round(bestGuess[4], 3)), str(timeRes), str(version))
#
...
elif modelNum == 7 and crop_hrs == 16:
...
elif modelNum == 7 and crop_hrs == 22:
...
#######################################################################################################
### GENERAL Function  (not for minimizing) to compute the -log(Likelihood) using MaxCal + ML estimation w/ Delayed Division       ###
#######################################################################################################
def MaxCal_MLe_general(lagrangeVals, nChromo):
    global factMat, cap
    probsList = [[[] for ind_g in range(cap)] for ind_C in range(NC_plot_max + 1)]
    logLike = 0.0
