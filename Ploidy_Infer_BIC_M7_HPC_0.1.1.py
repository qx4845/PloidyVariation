...
Cell Division (Ploidy) MaxCal w/ ML: 

    CellDiv_Inference+MonteCarlo Series:
        
        1.) Runs MaxCal Inference to extract Lagrange Multipliers
        2.) Uses extracted Lagrange Multipliers in a MaxCal Monte Carlo (MCMC) simulation
            to see what the extracted multipliers imply.
        3.) Plots results from 1.) and 2.) on top of each other with the following color convention:
            Data from 1.) is Blue; Data from 2.) is Red
    
    Version "CellDiv_Inference+MonteCarlo_0.6.5.py" Matches all Experimental (Blue) Graphs with those produced by
    version "ExperimentInfer_Orig_SW.py" as of 01 November 2018 -SW
    
    Version "CellDiv_Inference+MonteCarlo_0.7.py" computes Likelihood values and inferrs Lagrange Mutlipliers
    both of which match with Taylor's values. 
    
    Version "CellDiv_Inference+MonteCarlo_M2_0.9.py" successfully matches the first plot (# cells vs. time) with Taylor's. (12/5/2018, -SW)
    
            --> version "CellDiv_Inference+MonteCarlo_M2_0.9.2" fixes post processing issues in "CellDiv_Inference+MonteCarlo_M2_0.9.1"
            --> version "CellDiv_Inference+MonteCarlo_M2_0.9.2_alt_Old" (& series) is the same except uses my old MCMC routine.
            --> version "CellDiv_Inference+MonteCarlo_M2_0.9.3" (...does something to report here...?)
            
    The "CellDiv_Inference+MonteCarlo_M2_0.9.x.py" series attempts to match the average MCMC division time to that of experiment.
        --> completed with version "CellDiv_Inference+MonteCarlo_M2_0.9.3_div_times_fix_version_2"
    
    Version "CellDiv_Inference+MonteCarlo_M3_0.1.py" verifies that everything (Likelihood, Inference, Prediction) successfully 
            matches with Taylor's corresponding results for M3 (12/11/2018, -SW)
            
    Version "Ploidy_Likelihood_M4_0.1.py" attempts to mimic Taylor's results for Model 4 (M4), while also adjusting my notation to match his.
        --> This version extracts a -log(Likelihood) of 2096.8 which is 5.6 larger than Taylor's reported value: 2091.2  .
    
    Version "Ploidy_Likelihood_M4_0.2.py" attempts to match Taylor's M4 -log(Likelihood) = 2091.2 from his report dated "April 6, 2018".
        --> Both Stephen's version and Taylor's version (from "ExperimentInfer_P.py") of the MaxCal Likelihood function (see below) yield
            the same -log(Likelihood) = 2096.80095534 instead of the reported correct value of 2091.2  .
    
    Version "Ploidy_Likelihood_M4_0.3.1.py" changes the MaxCal_MLe function to only sum through "cap" instead of "cap +1" in line 385.
            This change does not change result from version "Ploidy_Likelihood_M4_0.3.py", which is: -log(Likelihood) = 2096.80095534 .
            
    Version "Ploidy_Likelihood_M4_0.3.2.py" changes the MaxCal_MLe function to only sum through "max_NC" instead of "max_NC +1". 
            This change results in some "list index out of range" error in the same funciton ... Attempting to resolve ...
            --> It turns out that Taylor uses max_NC = 200, which is >> 11 chromosomes, which is what is observed in these 12 movies (very first set).
                Since I use 11 instead of 200, I need to sum through "max_NC + 1" in order to allow the array sizes to be compatible with the function.
    
    Version "Ploidy_Likelihood_M4_0.4.1.py" neglects the likelihood contributions from cells that were already alive at the start of the movie 
    (maybe I should not do this until the Inference step...? -->  Yeah... it's wrong here. Try it if needed in the Inference step (NOPE). -SW
    
    Version "Ploidy_Likelihood_M4_0.4.4.py" SUCCESSFULLY reprodueces Taylor's result, and (together with version "Likelihood_typo_verified_ExperimentInfer_Cg_SW_modified_from_P_v1.py")
            proves that there is a typo on page 9 of Taylor's document dated April 6, 2018, h_gamma should be +0.088 (not negative). 
                This was and can be verified by taking version "Likelihood_typo_verified_ExperimentInfer_Cg_SW_modified_from_P_v1.py", 
                which was modified from Taylor's code for M5 (a.k.a. "Cg_P") and using the M5 Lagrange multipliers (LMs). This yields exactly the 
                reported -logL from Taylor's document. Now, when plugging in the M4 LMs, except changing h_gamma to positive, this now yields
                exactly the -logL from Taylor's document. (When using the reported  negative value for h_gamma (-0.088), this changes the associated 
                -logL to be wrong (this is where I was getting that 2096.80095534 value)). - SW 12/12/2018.
            
    Version "Ploidy_Likelihood_M5_0.1.1.py" SUCCESSFULLY reprodues Taylor's -logL value for M5 using his reported LMs.
    
    Version "Ploidy_Inference_M5_0.1.1.py" SUCCESSFULLY extracts the same LMs as reported by Taylor for M5. -SW 12/20/2018
    
    Version "Ploidy_Prediction_M5_0.1.1.py" SUCCESSFULLY reproduces the 5 plots for M5 from Taylor's document dated April 6, 2018,
            and also yields a division time that differs from experiment by (in the first realization) < 2 % (or about 1.889%).
            (Exp_AvgDivTime = 4.97288059701 hours    ;    MCMC_AvgDivTime_M5 = 5.06785098162 hours)  - SW 12/21/2018
            
    Version "Ploidy_Likelihood_M6_0.1.1.py" SUCCESSFULLY reprodues Taylor's -logL value for M6 using his reported LMs. -SW 12/21/2018
    
    Version "Ploidy_Inference_M6_0.1.1.py" SUCCESSFULLY extracts the same LMs as reported by Taylor for M6. -SW 12/21/2018
    
    Version "Ploidy_Prediction_M6_0.1.1.py" SUCCESSFULLY reproduces the 5 plots for M6 from Taylor's document dated April 6, 2018. -SW 12/21/2018
    
    Version "Ploidy_Likelihood_M7_0.1.1.py" SUCCESSFULLY reprodues Taylor's -logL value (when floored: Taylor:1995.8, Me:1995.89) for M6 using his reported LMs. -SW 12/21/2018
    
    Version "Ploidy_Inference_M7_0.1.1.py" SUCCESSFULLY extracts the same LMs as reported by Taylor for M7. -SW 12/21/2018

    Version "Ploidy_Prediction_M7_0.1.1.py" SUCCESSFULLY reproduces the 5 plots for M7 from Taylor's document dated April 6, 2018. -SW 12/21/2018
        Version "Ploidy_Prediction_M7_0.1.2.py" updates the filename to include 'hKgC2=%s' in the name of the save-file,
                                                and updates the loading and saving save-file locations for the new 2019 "Ghosh_group" file structure. -SW 01/03/2019
                                                
    All results (Models 1-7) from Taylor's document dated "April 6, 2018" have been completely successfully verified for 1.) Likelihood calculation, 2.) Lagrange Multiplier inference,
    and 3.) Forward prediciton using MaxCal Monte Carlo (MCMC) simulations. -SW 12/21/2018                                  
...
modelNum = 7
version = "Ploidy_Infer_BIC_M%s_HPC_0.1.1" %(str(int(modelNum)))

global h_alpha, h_A, K_A, maxLalpha, max_NA, t_f, numSims, timeRes, numInc, minNegLogLike

### Best Guesses for h_alpha, h_A, and K_A:
...
#h_K_Ca = -0.1                               ### Crosstalk                          ### 'h_K_gC' or 'h_K_gammaC'
...
##maxTime = int(2*86400) #604801      ### Total amount of time EXPERIMENTS were run (old #s)
...
##########################################################################################################
#### Crop data to include only first 15 hours:                                                         ###
##########################################################################################################
...
##########################################################################################################################
global NCG_max, max_NC
NCG_max = 0   
#for z in range(numTries):
#    for y in range(len(N_Chrom[z])):
#        if int(np.max(N_Chrom[z][y])) > NCG_max:
#            NCG_max = int(np.max(N_Chrom[z][y]))
for z in range(numTries):
        ...
        for nTry in range(len(N_C[nSim])):
            for cellNum in range(N_C[nSim][nTry].shape[0]):
                if np.isnan(N_C[nSim][nTry][cellNum, -1]):
                    div_times_2.append(np.sum(~np.isnan(N_C[nSim][nTry][cellNum, :])))
    return np.array(div_times_2)


div_times_exp = divisionTimes([numChrom])

print("Average Experimental Division Time: \n    " + str(np.average(np.multiply(div_times_exp, (timeRes/3600)))) + " hours" )

##            print("Division detected @ nTry = " + str(nTry) + ', tSlice = ' + str(frameInd) + ', cellNum = ' + str(splitCells[split_ind]))
#print('Total experimental divisions detected: ' + str(total_exp_divs) + '.')
#print('Total experimental division times analyzed: ' + str(total_analyzed_exp_divs) + '.')

#print('\nSelected (' + str(int(div_time_test_exp)) + '-hour) division time was observed ' + str(total_counted_divs_exp) + ' time(s) with a probability of ' + str(np.round(total_counted_divs_exp/len(div_times_exp), 3)) + " = " + str(np.round(total_counted_divs_exp/len(div_times_exp), 3)*100) + " %")

div_vals_exp, div_inds_exp =  np.histogram((timeRes/3600)*np.array(div_times_exp), bins = np.arange(0,48,1))
##divisionHist_Exp = np.histogram(divTimes_Exp,np.arange(0,np.max(divTimes_Exp) + 1,3600))[0]
#div_vals_exp = np.histogram(div_times_exp,np.arange(0,np.max(div_times_exp) + deltaT + 1,3600))[0]


#div_vals, div_inds1 =  np.histogram(np.array(div_times_exp), bins = np.arange(0,48,1))
...
def MaxCal_MLe(lagrangeVals):
    global factMat, cap, numChrom
    probsList = [[[] for ind_g in range(cap)] for ind_C in range(max_NC + 1)]
    logLike = 0.0
    for nTry in range(len(numChrom)):
        for cellNum in range(numChrom[nTry].shape[0]):
            if (np.sum(~np.isnan(numChrom[nTry][cellNum, :])) == 1 and ~np.isnan(numChrom[nTry][cellNum, -1])): ### If born on the last frame ...
                continue
#            if ~np.isnan(numChrom[nTry][cellNum, 0]):                                                   ### If already alive in the first frame...
#                continue
            startFrame = np.where(~np.isnan(numChrom[nTry][cellNum,:]))[0][0]
            endFrame = np.where(~np.isnan(numChrom[nTry][cellNum,:]))[0][-1] + 1
            probSum = []
            for gTind in range(2**(endFrame - startFrame)):                         ### Index numbering every possible gamma trajectory
                ...
                if endFrame == numChrom[nTry].shape[1]:                          ### If NO Division, i.e., if this cell is still alive at the end of the movie ...
                    if np.sum(lg_list) < cap:
                        probSum.append(1.0)
                        for frameNum in np.arange(startFrame, endFrame - 1, 1):
                            if len(probsList[int(numChrom[nTry][cellNum, frameNum])][int(np.sum(lg_list[:frameNum-startFrame]))]) == 0:
                                probsList[int(numChrom[nTry][cellNum, frameNum])][int(np.sum(lg_list[:frameNum-startFrame]))] = \
                                pathProbs(lagrangeVals, int(numChrom[nTry][cellNum, frameNum]), int(np.sum(lg_list[:frameNum-startFrame])))
                            tempProbs = probsList[int(numChrom[nTry][cellNum, frameNum])][int(np.sum(lg_list[:frameNum-startFrame]))]
#                            ipdb.set_trace() # Breakpoint
                            probSum[-1] *= tempProbs[int(numChrom[nTry][cellNum, frameNum + 1] - numChrom[nTry][cellNum, frameNum]), \
                                   int(np.sum(lg_list[:frameNum - startFrame + 1])), int(numChrom[nTry][cellNum, frameNum + 1])]
...
#                            ipdb.set_trace() # Breakpoint
                            ...
                            motherInds = np.where(np.all([~np.isnan(numChrom[nTry][:, frameNum]), np.isnan(numChrom[nTry][:, frameNum + 1])], axis=0))[0]
                            daughterInds = np.where(np.all([np.isnan(numChrom[nTry][:, frameNum]), ~np.isnan(numChrom[nTry][:, frameNum + 1])], axis=0))[0]
                            cellInd = np.where(motherInds == cellNum)[0][0]
#                            ipdb.set_trace() # Breakpoint
                            probSum[-1] *= tempProbs[int(numChrom[nTry][daughterInds[2 * cellInd], frameNum + 1] + numChrom[nTry][daughterInds[2 * cellInd + 1], frameNum + 1] - numChrom[nTry][motherInds[cellInd], frameNum]), \
                                   int(np.sum(lg_list[:frameNum - startFrame + 1])), int(numChrom[nTry][daughterInds[2 * cellInd], frameNum + 1])]
...
#print("Initial guess Lagrange multipliers: [h_C, h_g, h_P, h_KgC, h_KgC2] = " + str(firstGuess) + "\nwith corresponding -log(Likelihood) = " + str(initLogLike))
...
#######################################################################################################
### For testing Likelihood calculation for M7 (12/21/2018 -SW): 
print("Computing the Likelihood ...")
likeStartTime = time.time()
Lagr01 = MaxCal_MLe(MC_lagrange)             ## Stephen's version   
#bestGuess = MC_lagrange
#bestLike = Lagr01
    
expTrajLens = []
for nTry in range(len(numChrom)):
    expTrajLens.append(numChrom[nTry].shape[1])
avgExpTrajLen = np.average(expTrajLens)
totalExpTrajLen = np.sum(expTrajLens)
    
###BIC = len(bestGuess.tolist())*np.log(numChrom.shape[1]) - 2*np.log(bestLike[0])
###BIC = len(bestGuess.tolist())*np.log(2*crop_hrs) - 2*np.log(bestLike[0])
##BIC = len(bestGuess.tolist())*np.log(2*avgExpTrajLen) - 2*np.log(bestLike[0])
#BIC = len(bestGuess.tolist())*np.log(2*totalExpTrajLen) - 2*np.log(bestLike[0])
BIC = len(MC_lagrange)*np.log(2*totalExpTrajLen) - 2*np.log(Lagr01)


print("\n**************************************************************")
print('Lagrange Mutlipliers: \n[h_C, h_g, h_P, h_KgC, h_KgC2] = ' + str(MC_lagrange) + ' \nyield -log(Likelihood) = ' + str(Lagr01) + ", and BIC = " + str(BIC) + '\n')

#print('h_C = ' + str(round(lagrangeVals[0],3)) + ', h_g = ' + str(round(lagrangeVals[1],3)) + ', h_P = ' + str(round(lagrangeVals[2],3)) + ', cap = ' + str(cap) + ', loglike = ' + str(round(loglike,1)))
likeCalcTime = time.time() - likeStartTime
print("Likelihood computation took: %s secs" % timedelta(seconds=round(likeCalcTime)))
#if Lagr01 < 2091.1:
#    print("... too low ... maybe try something else...?\n")
#elif 2091.1 < Lagr01 < 2091.3:
#    print("which SUCCESSFULLY matches Taylor's result !!!\n")
#elif Lagr01 < 2093:
#    print("    ... almost there ... You got this!!!\n")
#elif Lagr01 < 2095:
#    print("    ... getting a little bit closer ... Keep it up!\n")
#if Lagr01 > 2096.80095534:
#    print("    ... larger than before ... maybe try something else...?\n")

print("with Version: " + str(version) + '\n')
#print("**************************************************************\n")
print("**************************************************************")

###print("\nOptimized Lagrange Multipliers: \n[h_C, h_g, h_P, h_KgC, h_KgC2] = " + str(bestGuess) + "\nwith corresponding -log(Likelihood) = " + str(bestLike[0]) + ", and BIC = " + str(BIC))
##print("\nOptimized Lagrange Multipliers: \n[h_C, h_g, h_P, h_KgC] = " + str(bestGuess) + "\nwith corresponding -log(Likelihood) = " + str(bestLike[0]) + ", and BIC = " + str(BIC))
#print("\nOptimized Lagrange Multipliers: \n[h_C, h_g, h_P, h_KgC] = " + str(MC_lagrange) + "\nwith corresponding -log(Likelihood) = " + str(bestLike[0]) + ", and BIC = " + str(BIC))
#
#filename_inference = '/home/swedeki3/PloidyVariation/BIC/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_hP=%s_hKgC=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(np.round(bestGuess[2], 3)), str(np.round(bestGuess[3], 3)), str(timeRes), str(version))
##filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/BIC_Comparison/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_hP=%s_hKgC=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(np.round(bestGuess[2], 3)), str(np.round(bestGuess[3], 3)), str(timeRes), str(version))
#np.savez(filename_inference, bestGuess = bestGuess, bestLike = bestLike, BIC = BIC, bestInd = bestInd, fitStartGuess = fitStartGuess, fitLagrangeVals = fitLagrangeVals, fitLogLike = fitLogLike)

filename_inference = '/home/swedeki3/PloidyVariation/BIC/M%s/Ploidy_LikelihoodTest_Results_hC=%s_hg=%s_hP=%s_hKgC=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(MC_lagrange[0], 3)), str(np.round(MC_lagrange[1], 3)), str(np.round(MC_lagrange[2], 3)), str(np.round(MC_lagrange[3], 3)), str(timeRes), str(version))
#filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/BIC_Comparison/M%s/Ploidy_LikelihoodTest_Results_hC=%s_hg=%s_hP=%s_hKgC=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(MC_lagrange[0], 3)), str(np.round(MC_lagrange[1], 3)), str(np.round(MC_lagrange[2], 3)), str(np.round(MC_lagrange[3], 3)), str(timeRes), str(version))
#filename_inference = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/BIC_Comparison/M%s/Ploidy_Inference_Results_hC=%s_hg=%s_hP=%s_hKgC=%s_deltaT=%s_v%s' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(np.round(bestGuess[2], 3)), str(np.round(bestGuess[3], 3)), str(timeRes), str(version))
#np.savez(filename_inference, bestGuess = bestGuess, bestLike = bestLike, BIC = BIC, bestInd = bestInd, fitStartGuess = fitStartGuess, fitLagrangeVals = fitLagrangeVals, fitLogLike = fitLogLike)
np.savez(filename_inference, MC_lagrange = MC_lagrange, Lagr01 = Lagr01, BIC = BIC, div_times_exp = div_times_exp)
...
###################################################################################################### 
#print("**************************************************************")
#print("Starting MaxCal Monte Carlo (MCMC) Prediction Series ...")
#print("**************************************************************")
#  
...
#    global factMat, num_MCMC_trials, maxT, max_NC, NC_init, NG_init
...
#
#        filename001 = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/__PROJECTS/PloidyVariation/Project2/M%s/Ploidy_MCMC_trajectories_hC=%s_hg=%s_hP=%s_hKgC=%s_hKgC2=%s_deltaT=%ss_v%s.npz' %(str(int(modelNum)), str(np.round(bestGuess[0], 3)), str(np.round(bestGuess[1], 3)), str(np.round(bestGuess[2], 3)), str(np.round(bestGuess[3], 3)), str(np.round(bestGuess[4], 3)), str(timeRes), str(version))
#
###        sio.savemat(filename0, {'N_A': N_A, 'N_alpha_star': N_alpha_star, 'R_A': R_A, 'RXNs': RXNs})
#        np.savez(filename001, nC_MCMC = nC_MCMC, maxT = maxT, numTrials = numTrials, num_MCMC_trials = num_MCMC_trials, total_time_seconds = t_f, timeRes = timeRes, num_chrom_in_div_cell_mcmc = num_chrom_in_div_cell_mcmc)
#
...
##filename_div_times = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/MaxCal_ChromosomeCopying_CellDividing_ChromosomePartitioning/MaxCal_Inference_vs_MaxCal_Monte_Carlo/CellDiv_MaxCal_Inference+MonteCarlo_comparison_hC=%s_ha=%s_hA=%s_deltaT=%s_v%s.npz' %(str(np.round(MC_lagrange[0], 3)), str(np.round(MC_lagrange[1], 3)), str(np.round(MC_lagrange[2], 3)), str(timeRes), str(version))
###filename_div_times = '/Users/stephenwedekind/Dropbox/School/RESEARCH/Ghosh_group/MaxCal_ChromosomeCopying_CellDividing_ChromosomePartitioning/MaxCal_Inference_vs_MaxCal_Monte_Carlo/CellDiv_MaxCal_Inference+MonteCarlo_comparison_hC=%s_ha=%s_deltaT=%s_v%s.npz' %(str(np.round(MC_lagrange[0], 3)), str(np.round(MC_lagrange[1], 3)), str(timeRes), str(version))
###    sio.savemat(filename0, {'N_A': N_A, 'N_alpha_star': N_alpha_star, 'R_A': R_A, 'RXNs': RXNs})
